...
export const signinUser: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) return next(createHttpError(404, "Пользователь не найден!"));
    if (!user.isUserVerified)
      return next(createHttpError(406, "Пользователь не проверен"));
...
export const sendVerificationMail: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if (user.isUserVerified)
      return next(createHttpError(406, "Пользователь уже проверен"));
...
export const verifyUserMail: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) return next(createHttpError(401, "Токен недействителен"));
...
export const sendForgotPasswordMail: RequestHandler = async (
  ...
  try {
    const user = await User.findOne({ email });
    if (!user) return next(createHttpError(404, "Электронная почта недействительна!"));
